...
def sync():
    try:
        from .sync_utils import sync_data  # import only when route is used
        synced = sync_data(current_user.id)  # sync only this user's data
        flash(f"✅ Sync complete: {synced} records updated.", "success")
    except Exception as e:
